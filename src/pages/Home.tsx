...
function Home() {
  ...
  <p className="mb-8 text-base sm:text-lg text-sky-500 max-w-2xl mx-auto font-medium">
          <span className="leading-relaxed">스마트한의 주차, IoT, 영상 통합 솔루션</span>
        </p>
        <div
        ...
        <div className="flex flex-col items-center my-8 sm:my-10">
          <Button
            className="px-6 py-2 text-base font-medium rounded-full border border-sky-500 bg-white text-sky-500 shadow-sm hover:bg-sky-500 hover:text-white focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:outline-none transition-all duration-200 flex items-center gap-2"
            aria-label="상담 문의하기"
            onClick={() => setModalOpen(true)}
          >
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#f1f5fe"/><path d="M8 12h8M12 8v8" stroke="#38bdf8" strokeWidth="2" strokeLinecap="round"/></svg>
            상담 문의하기
          </Button>
        </div>
        <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
          <h2 className="text-xl font-bold mb-4 text-sky-500">상담 문의하기</h2>
          <form className="flex flex-col gap-4">
            ...
            <button type="submit" className="bg-sky-500 text-white font-semibold rounded py-2 mt-2 hover:bg-sky-400 transition">문의 보내기</button>
          </form>
        </Modal>
      </section>
    </>
