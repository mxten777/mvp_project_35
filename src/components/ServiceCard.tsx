...
export default function ServiceCard({ icon, title, description, className, children }: ServiceCardProps) {
  return (
    <div 
      className={cn(
  "flex flex-col items-center text-center p-4 m-0 border-0 outline-none w-full h-full rounded-lg transition-all duration-300 hover:bg-emerald-50",
        className
      )} 
      style={{ border: 'none !important', outline: 'none !important', boxShadow: 'none !important' }}
    >
  <div className="mb-4 text-emerald-600 border-0 outline-none p-3 bg-emerald-50 rounded-full flex items-center justify-center w-16 h-16">{icon}</div>
...
export const ServiceIcons = {
  parking: <LucideParkingCircle size={36} strokeWidth={1.5} className="text-emerald-500" />,
  iot: <LucideCpu size={36} strokeWidth={1.5} className="text-emerald-500" />,
  cctv: <LucideCamera size={36} strokeWidth={1.5} className="text-emerald-500" />,
};
