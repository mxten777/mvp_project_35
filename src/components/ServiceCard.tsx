...
export default function ServiceCard({ icon, title, description, className, children }: ServiceCardProps) {
  return (
    <div 
      className={cn(
  "flex flex-col items-center text-center p-4 m-0 border-0 outline-none w-full h-full rounded-lg transition-all duration-300 hover:bg-violet-50",
        className
      )} 
      style={{ border: 'none !important', outline: 'none !important', boxShadow: 'none !important' }}
    >
  <div className="mb-4 text-violet-600 border-0 outline-none p-3 bg-violet-50 rounded-full flex items-center justify-center w-16 h-16">{icon}</div>
      <h3 className="font-bold text-lg mb-2 text-[#222] border-0 outline-none leading-tight">{title}</h3>
