...
export default function ServiceCard({ icon, title, description, className, children }: ServiceCardProps) {
  return (
    <div 
      className={cn(
        "flex flex-col items-center text-center p-4 m-0 border-0 outline-none w-full h-full rounded-lg transition-all duration-200 hover:bg-violet-50 hover:shadow-md hover:-translate-y-1 focus-within:shadow-md focus-within:-translate-y-1 bg-white/90",
        className
      )} 
      style={{ border: 'none !important', outline: 'none !important', boxShadow: 'none !important' }}
    >
  <div className="mb-4 text-violet-600 border-0 outline-none p-3 bg-violet-50 rounded-full flex items-center justify-center w-16 h-16 transition-all duration-200 group-hover:scale-105">{icon}</div>
